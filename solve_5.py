...
#        print('one:     ' + str(tmp_one))
#        print('two:     ' + str(tmp_two))
#        print('three:   ' + str(tmp_three))
#        print('corners: ' + str(tmp_corners))

        if tmp_three == 4:
            tmp_loop = False

        if tmp_three > 0 and tmp_corners < 3:
#            print('look for three')
            solve_three()
...
def debug_image():
    ...
    print(pieces[36] + pieces[37] + pieces[38] + " " + pieces[0] + pieces[1] + pieces[2] + " " + pieces[45] + pieces[46] + pieces[47])
    print(pieces[39] + pieces[40] + pieces[41] + " " + pieces[3] + pieces[4] + pieces[5] + " " + pieces[48] + pieces[49] + pieces[50])
    print(pieces[42] + pieces[43] + pieces[44] + " " + pieces[6] + pieces[7] + pieces[8] + " " + pieces[51] + pieces[52] + pieces[53])
...
def solve_corners():
    ...
    for t_index, t_edge in enumerate(tmp_edges):
        ...
        if (pieces[t_edge[0]] == pieces[t_edge[2]] and
            pieces[t_edge[0]] != pieces[t_edge[1]]):
#            print('find one corner set')

            tmp_count = 0
            for i_index, i_edge in enumerate(tmp_edges):

                if pieces[i_edge[0]] == pieces[i_edge[2]]:
                    tmp_count = tmp_count + 1

            if tmp_count == 4:
#                print('find 4 matching corners')
                move_edge(t_index, t_edge)

                if pieces[9] == pieces[39]:
#                    print('find mid green left')
                    find_mid_0(0)
                elif pieces[9] == pieces[50]:
#                    print('find mid green right')
                    find_mid_0(1)
                elif pieces[36] == pieces[10]:
#                    print('find mid red left')
                    find_mid_2(0)
                elif pieces[36] == pieces[34]:
#                    print('find mid red right')
                    find_mid_2(1)
                elif pieces[47] == pieces[10]:
#                    print('find mid orange left')
                    find_mid_3(0)
                elif pieces[47] == pieces[34]:
#                    print('find mid orange right')
                    find_mid_3(1)
                elif pieces[33] == pieces[39]:
#                    print('find mid blue left')
                    find_mid_1(0)
                elif pieces[33] == pieces[50]:
#                    print('find mid blue right')
                    find_mid_1(1)
                else:
#                    print('default mid green left')
                    find_mid_0(0)

                break

            if tmp_count == 1:
#                print('find 1 matching corners')
                move_edge(t_index, t_edge)
...
def solve_three():
    ...
    for t_index, t_edge in enumerate(tmp_edges):
        ...
        if (pieces[t_edge[0]] == pieces[t_edge[1]] and
            ...
            if tmp_count == 4:
#                print('find all threes in a row')
                move_edge(t_index, t_edge)
                break
...
#        print('EDGE: ' + pieces[tmp_edge[0]])
#        print('INDEX: ' + str(tmp_index))
